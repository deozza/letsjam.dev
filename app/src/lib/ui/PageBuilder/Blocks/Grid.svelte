...
<div class={['grid', 'gap-6', element.columns !== undefined && element.columns !== null && `md:grid-cols-${element.columns}`, element.rows !== undefined && element.rows !== null && `md:grid-rows-${element.rows}`, `self-${selfAlignmentMapping.get(element.alignement)}` || '', 'place-content-stretch']}>
